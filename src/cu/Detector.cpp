...
Detector::Detector(int NUM_CLASSES, const std::string &TARGET_COLOUR, float NMS_THRESH, float BBOX_CONF_THRESH,
                   int INPUT_W, int INPUT_H, const std::string &engine_file_path, float min_large_center_distance)
    : NUM_CLASSES(NUM_CLASSES), TARGET_COLOUR(TARGET_COLOUR), NMS_THRESH(NMS_THRESH), BBOX_CONF_THRESH(BBOX_CONF_THRESH),
      INPUT_W(INPUT_W), INPUT_H(INPUT_H), engine_file_path(engine_file_path), min_large_center_distance(min_large_center_distance)
{
    InitModelEngine();
    AllocMem();
}
...
void Detector::InitModelEngine()
{
    ...
    char *trtModelStreamDet = new char[size];
...
std::vector<Armor> Detector::detect(cv::Mat &frame)
{
    cudaStreamCreate(&stream);

    // 预处理
    AffineMatrix afmt(cv::Size(INPUT_W, INPUT_H), cv::Size(frame.cols, frame.rows));
    ...
    std::size_t img_size = 3 * frame.rows * frame.cols;
    ...
    preprocess(
        img_device, frame.cols, frame.rows,
        buffers[inputIndex], INPUT_W, INPUT_H,
    ...
    decode_kernel_invoker(
        buffers[outputIndex], NUM_BOX_ELEMENT, OUTPUT_CANDIDATES, NUM_CLASSES,
    ...
    int count = std::min(int(decode_ptr_host[0]), MAX_OBJECTS);
    if (count)
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            {
                box.landmarks[j << 1] = decode_ptr_host[landmark_pos + (j << 1)];
                box.landmarks[j << 1 | 1] = decode_ptr_host[landmark_pos + (j << 1 | 1)];
            }
        ...
        for (auto box : boxes)
        {
            ...
            Armor armor(points);
            float light_left_length = std::abs(box.landmarks[1] - box.landmarks[3]);
            float light_right_length = std::abs(box.landmarks[7] - box.landmarks[5]);
            ...
            if (this->TARGET_COLOUR == "BLUE" && box.class_id >= 0 && box.class_id <= 8)
            {
                armor.type = center_distance > min_large_center_distance ? ArmorType::LARGE : ArmorType::SMALL;
                if (box.class_id == 0)
                    armor.number = "guard";
            ...
            else if (this->TARGET_COLOUR == "RED" && box.class_id >= 9 && box.class_id <= 17)
            {
                armor.type = center_distance > min_large_center_distance ? ArmorType::LARGE : ArmorType::SMALL;
                if (box.class_id == 9)
                    armor.number = "guard";
        ...
        //                                     cv::Point2f(box.landmarks[4], box.landmarks[5]),
        //                                     cv::Point2f(box.landmarks[6], box.landmarks[7])};
        //     for (int i = 0; i < CKPT_NUM; ++i)
        //         cv::circle(frame, points[i], 2, color_list[i], -1);
        //     std::string label = std::to_string(box.class_id);
        //     cv::line(frame, points[0], points[2], color_list[1], 2);
        //     cv::line(frame, points[1], points[3], color_list[1], 2);
        //     cv::line(frame, points[0], points[1], color_list[1], 2);
        //     cv::line(frame, points[2], points[3], color_list[1], 2);
        //     cv::putText(frame, label, points[0] - cv::Point(0, 2), cv::FONT_HERSHEY_PLAIN, 2, color_list[1], 2);
        // }
    }
